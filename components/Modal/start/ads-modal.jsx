      ...
      >
        {slides?.data?.storyImages?.map((item, index) => {
          return (
            <SwiperSlide
              key={index}
              className={`w-full h-full relative z-0 bg-grey-7 after:absolute after:w-full after:h-full after:bg-black ${
                item?.description ? "after:opacity-50" : "after:opacity-0"
              } after:top-0 after:left-0 after:z-0`}
              style={{
              ...
              <div className="w-11/12 small:w-[85%] mx-auto h-full flex flex-col items-center justify-end gap-2 text-white text-center pb-14 relative z-10">
                ...
                {item?.button_text ? (
                  <a
                    ...
                    className="block py-3 px-20 rounded-3xl font-medium text-base linear"
                  >
                    {item?.button_text}
                  </a>
                ) : (
